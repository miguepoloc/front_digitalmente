...
const ModuloPiensalo = () => {
    ...
    useEffect(() => {
        ...
        const controlData = async () => {
            ...
            if (!AvanceState) {
                ...
                if (parseInt(slug) > AvanceState.piensalo
                    &&
                    parseInt(slug) !== linksPiensalo.length - 1
                ) {
                    history.push(`/dashboard`)
                } else {
                    if (AvanceState.piensalo === linksPiensalo.length - 1 && parseInt(slug) === linksPiensalo.length - 1) {
                        const jsonx = {
                            piensalo: (parseInt(slug) + 1),
                            usuario: userInfo.id
                        }
    ...
    useEffect(() => {
        ...
        if (parseInt(slug) !== linksPiensalo.length - 1) {
            setBotonState(false)
        }
    }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    async function cambioBotonAdelante() {
        ...
        if (parseInt(slug) === linksPiensalo.length - 2 && AvanceState.piensalo === linksPiensalo.length - 2) {
        ...
        if ((linksPiensalo.length - 1) === parseInt(slug)) {
            history.push(`/dashboard`)
        }
        else {
            history.push(`/piensalo${parseInt(slug) + 1}`)
            setLoading(true)
        }
                ...
                <main className="main-content position-relative h-100 border-radius-lg ">
                    ...
                    <div className="container-fluid py-4">
                        ...
                        <div >
                            ...
                            {
                                loading ?
                                    (linksPiensalo[slug - 1].tipoCapsula
                                        ? <img src={linksPiensalo[slug - 1].imagen} alt="capsula" className='img-capsula' />
                                        : linksPiensalo[slug - 1].actividad)
                                    :
                                    <Loading />
                            }
                        ...
                        <div className='d-flex justify-content-center justify-content-sm-between  flex-wrap'>
                            ...
                            <button
                                ...
                                disabled={AvanceState.piensalo < linksPiensalo.length - 1 && parseInt(slug) === linksPiensalo.length - 1 ? true : BotonAtrasState}
                            >
                            ...
                            {parseInt(slug) === linksPiensalo.length - 1 ? (
                                <button
                                    ...
                                    onClick={() => { if (AvanceState.piensalo < linksPiensalo.length - 1 && parseInt(slug) === linksPiensalo.length - 1) { history.push(`/dashboard`) } else { ; history.push(`/dashboard`) } }}
                                    disabled={false}
                                >
                                    Regresar  <AiFillHome
